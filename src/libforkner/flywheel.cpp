...
void _velocityMonitor(void* ign) {
	...
	while (true) {
		...
		if (flywheelVelocities[velocityIndex] != prevDesVel) tbhCalcFlag = false;
...
void start() {
	if (!initialized) {
		...
		indexerMotor =
		    std::make_unique<okapi::Motor>(INDEXER, false, okapi::AbstractMotor::gearset::blue,
		                     okapi::AbstractMotor::encoderUnits::degrees);
		indexerMotor->setBrakeMode(okapi::AbstractMotor::brakeMode::hold);
		intakeMotor =
		    std::make_unique<okapi::Motor>(INTAKE, true, okapi::AbstractMotor::gearset::green);
...
void shootBlocking() {
	if (logger) logger->log("F, Firing Once\n");
	shotMutex->take(20);
...
void doubleShootBlocking(bool noDelay) {
	if (logger) logger->log("F, Firing Twice\n");
	doubleShootMutex->take(20);
	doubleShootFlag = true;
	doubleShootMutex->give();
...
void indexOne(int intakePower) {
	if (logger) logger->log("F, Indexing a Ball\n");
	indexMutex->take(20);
	indexFlag = 1;
	indexMutex->give();
}
...
bool indexerCheck() {
	return indexerPhotogate.get_value();
}
...
void setIntakeLift(bool state) {
	intakeLiftSolenoid->set_value(state);
}

void setIntakeCurrentLimit(int limit) {
	intakeMotor->setCurrentLimit(limit);
}

void setAngleChanger(bool val) {
	angleChanger->set_value(val);
}

const char* setVelocitySetpoint(int velIndex) {
	if (logger) logger->log("F, Setting the %d Velocity Setpoint\n", velIndex);
